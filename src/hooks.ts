...
function useTimeCacheValue<T>(key: string): [GetItem<T>, SetItem<T>] {
	...
	return [
		...
		(item: T) => {
			localStorage.setItem(key, JSON.stringify({ saved: Date.now(), value: item } as ITimeItem<T>))
		},
	]
}

export function useBioRepos(gistId: string): [string | undefined , string[] | undefined] {
			...
			fetch(`https://api.github.com/gists/${gistId}`)
				...
				.then((r) => {
					if (r && r.files)
					{
						...
						const repos = r.files.Repos.content.split('\n').filter((repo: string) => repo.length).filter((repo: string) => repo.includes('/'))
						saveRepos(repos)
						setRepos(repos)
					}
				})
...
export function useProjects(projectsIds: string[]): Project[] {
	...
	useEffect(() => {
		if (projects === undefined || projects.length !== projectsIds.length) {
			...
			Promise.all(projectsIds.map((projectId) => fetch(`https://api.github.com/repos/${projectId}`))).then((repos) =>
				Promise.all(repos.map((repo) => repo.json())).then((informations: GitHubProjects[]) => {
